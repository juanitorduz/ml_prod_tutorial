# Resources

## Main References:

Blog posts from [https://mlinproduction.com/](https://mlinproduction.com/), specially

- [Docker for Machine Learning – Part I](https://mlinproduction.com/docker-for-ml-part-1/)
- [Docker for Machine Learning – Part II](https://mlinproduction.com/docker-for-ml-part-2/)
- [Docker for Machine Learning – Part III](https://mlinproduction.com/docker-for-ml-part-3/)
- [Using Docker to Generate Machine Learning Predictions in Real Time](https://mlinproduction.com/docker-for-ml-part-4/).

## Miscellaneous

### Similar Approaches:

#### Python

- [Expose an API for a trained prediction model](http://drorata.github.io/posts/2018/May/08/expose-an-api-for-a-trained-prediction-model/)
- [https://github.com/tanujjain/deploy-ml-model](https://github.com/tanujjain/deploy-ml-model)

#### R

- [Introduction to R Plumber : Expose a Caret model to a web API](https://juanitorduz.github.io/intro_plumber/)

### Virtual Environments

- [How-To: Building a Virtual Python Environment](https://developer.akamai.com/blog/2017/06/21/how-building-virtual-python-environment)
- [Elegantly activating a virtualenv in a Dockerfile](https://pythonspeed.com/articles/activate-virtualenv-dockerfile/)

### Testing

- [Testing Flask Applications](https://flask.palletsprojects.com/en/1.1.x/testing/)

### AWS Credentials in Docker

- [Getting AWS Credentials into a Docker Container without Hardcoding It](https://cameroneckelberry.co/words/getting-aws-credentials-into-a-docker-container-without-hardcoding-it)
- [Detect Greek language in text using Docker, & store in S3 without Boto3](https://towardsdatascience.com/detect-greek-language-in-text-using-docker-store-in-s3-without-boto3-3c6d6b220639)
- [Stackoverflow: AWS credentials in Dockerfile](https://stackoverflow.com/questions/51824671/aws-credentials-in-dockerfile/51825625#51825625)

### GitHub Actions

- [CI/CD for Docker Using GitHub Actions](https://medium.com/better-programming/github-actions-for-docker-eaf22bbcc879)
